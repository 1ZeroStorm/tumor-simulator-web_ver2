...
def create_tumor_visualization(tumor_size, resistance_list, max_res=15.0): 
    ...
    fig = go.Figure(
        data=[
            go.Scattergl(
                x=cell_coords[:, 0], # 1D array of x coords of every cell 
                y=cell_coords[:, 1], # 1D array of y coords of every cell
                mode='markers',
                marker=dict(
                    size=5,
                    color=norm_colors,
                    colorscale='YlOrRd',  # yellow= low, orange= med, red = high
                    cmin=0, # clipping the colors, checking norm_colors if theres below 0, keep the same color as 0, this applied also to above 1
                    cmax=1,
                    showscale=True,
                    colorbar=dict( 
                    ...
                    opacity=0.8 # 80% solid, 20% transparent
                ),
                hovertemplate='Cell Resistance: %{marker.color:.2f}<extra></extra>' # adding template cell resistance instead of just coordinates
            )
        ]
    )
    ...
    return fig # returns this, thus the calling just have to use .show()
